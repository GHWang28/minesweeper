...
const generateColor = (inputData) => {
  if (!inputData.isGameOver) return 'rgb(113,222,73)';
  
  const charred = Math.max(20, 110 - inputData.charred * 10);
  return `rgb(${charred},${charred},${charred})`
}

export default function Cell ({ onClick, data, hoveringCell }) {
  ...
  const transitions = useTransition(data.isSweeped, {
    ...
    delay: 100 * calcDistance(data, hoveringCell)
  })
  ...
  return transitions((style, dataTransition) => (
    <AnimatedBox
      ...
      sx={{
        ...
        bgcolor: generateColor(data),
        filter: (dataTransition) ? 'brightness(0.75)' : 'brightness(1)',
        cursor: 'pointer',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
        backgroundImage: generateIcon(data, dataTransition, flagged),
